...
template<typename Line>
Groups<Line> FilterByAngle(const Groups<Line> &groups)
{
    Groups<Line> result;
...
template<typename Line>
Groups<Line> SplitMissingLines(const Groups<Line> &groups, double ratioLimit)
{
    Groups<Line> splitOnMissingLines;
    ...
    double verticalAngle)
{
    ...
    groups.sort(
        [&](const LineGroup<Line> &first, const LineGroup<Line> &second) -> bool
        {
...
template<typename Line>
void CombineFirstAndLast(Groups<Line> &groups, double groupSeparation_deg)
{
...
template<typename Line>
AxisGroups<Line> SelectAxisGroups(
    Groups<Line> &groups,
    double minimumSpacing,
    bool allowSkippedLines)
{
    ...
    Groups<Line> theRest(group, end(groups));
    ...
    [[maybe_unused]] double maximumPointError)
{
    ...
    for (auto j: jive::Range<size_t>(0, horizontalCount))
    {
        ...
        {
            horizontalPoints = horizontalLine.GetPoints();
        }
