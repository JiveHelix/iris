...
template<template<typename> typename G, typename L>
std::ostream & operator<<(
    std::ostream &outputStream,
    const std::list<G<L>> groups)
{
    size_t count = 0;
...
struct LineGroup
{
    ...
    std::vector<LineGroup> SplitOnSorting()
    {
        ...
        bool isDescending = std::signbit(
            tau::LineAngleDifference(
                this->lines[1].GetAngleDegrees(),
...
extern template struct LineGroup<tau::Line2d<double>>;
extern template struct LineGroup<LineFromPoints>;
