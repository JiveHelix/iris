...
WaveformPixels::WaveformPixels(
    ...
    waveformPixels_(this, pixelsControl),
    waveformControl_(waveformControl),
    waveformData_(),
    image_(
        static_cast<int>(this->waveformControl_.displayedSize.Get().width),
        static_cast<int>(this->waveformControl_.displayedSize.Get().height))
{
    this->Bind(wxEVT_PAINT, &WaveformPixels::OnPaint_, this);
    this->Bind(wxEVT_SIZE, &WaveformPixels::OnSize_, this);
    this->waveformPixels_.Connect(&WaveformPixels::OnPixels_);
...
void WaveformPixels::OnPixels_(const Pixels &waveform)
{
...
void WaveformPixels::OnSize_(wxSizeEvent &event)
{
    ...
    this->waveformControl_.displayedSize.Set(imageSize);
}
...
WaveformView::WaveformView(
    ...
    waveformPixels_(
        new WaveformPixels(this, pixelsControl, waveformControl)),
    waveformTerminus_(this, waveformControl)
{
    this->Bind(wxEVT_PAINT, &WaveformView::OnPaint_, this);

    this->waveformTerminus_.columnCount.Connect(
        &WaveformView::OnWaveformSize_);

    this->waveformTerminus_.levelCount.Connect(
        &WaveformView::OnWaveformSize_);

    this->waveformTerminus_.verticalScale.Connect(
