...
{
    auto intrinsics = iris::Intrinsics<float>::Default();

    iris::Pose<float> pose(
        {
...
{
    auto intrinsics = iris::Intrinsics<float>::Default();
    auto pose = iris::Pose<float>::Default();

    iris::Vector3<float> world(5.0_f, 0.0_f, 0.0_f);
    iris::Projection projection(intrinsics, pose);
    iris::Vector3<float> pixel = projection.WorldToCamera(world);
...
TEST_CASE("Shifted world to Camera", "[projection]")
{
    ...
    auto noShift = iris::Pose<float>::Default();

    auto pose = iris::Pose<float>::Default();
    pose.y_m = 1.0_f;

    iris::Vector3<float> world(5.0_f, 0.0_f, 0.0_f);
    iris::Projection noShiftProjection(intrinsics, noShift);
    iris::Projection projection(intrinsics, pose);

    iris::Vector3<float> noShiftPixel =
